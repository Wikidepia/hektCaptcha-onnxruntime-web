...
exports.conv = exports.parseConvAttributes = exports.calculateOutputShape = void 0;
const util_1 = require("../../util");
const attribute_with_cache_key_1 = require("../attribute-with-cache-key");
const conv_grouped_1 = require("./conv-grouped");
const conv2d_mm_1 = require("./conv2d-mm");
const fuse_utils_1 = require("./fuse-utils");
const transpose_1 = require("./transpose");
const calculateOutputShape = (inputShape, kernelShape, dilations, adjustPads, strides, isChannelLast) => {
...
const weightTransposeAttribute = (0, attribute_with_cache_key_1.createAttributeWithCacheKey)({ perm: [2, 3, 1, 0] });
...
const getAdjustedConvAttributes = (attributes, inputs) => {
    ...
    util_1.PoolConvUtil.adjustPadsBasedOnAutoPad(inputs[0].dims, attributes.strides, attributes.dilations, kernelShape, pads, attributes.format === 'NHWC', attributes.autoPad);
    // always return a new object so does not modify the original attributes
    const newAttributes = Object.assign({}, attributes);
    Object.assign(newAttributes, { kernelShape, pads, cacheKey: attributes.cacheKey });
    return newAttributes;
};
const parseConvAttributes = (attributes) => {
    ...
    return (0, attribute_with_cache_key_1.createAttributeWithCacheKey)({ autoPad, format, dilations, group, kernelShape, pads, strides, wIsConst, ...activationAttributes });
};
...
const conv2d = (context, inputs, attributes) => {
    ...
    const outputShape = (0, exports.calculateOutputShape)(inputs[0].dims, inputs[1].dims, attributes.dilations, adjustedAttributes.pads, attributes.strides, isChannelsLast);
    const outHeight = outputShape[isChannelsLast ? 1 : 2];
    const outWidth = outputShape[isChannelsLast ? 2 : 3];
    const outChannels = outputShape[isChannelsLast ? 3 : 1];
    const sameSize = isChannelsLast && weightHeight === inputHeight && weightWidth === inputWidth && attributes.autoPad === 'VALID';
    if (sameSize ||
        ...
        context.compute((0, conv_grouped_1.createGroupedConvProgramInfoLoader)(inputs, adjustedAttributes));
        return;
    }
    ...
    const transposedWeight = context.customData.wT ??
        context.compute({
            ...transpose_1.transposeProgramMetadata,
            cacheHint: weightTransposeAttribute.cacheKey,
            get: () => (0, transpose_1.createTransposeProgramInfo)(inputs[1], weightTransposeAttribute.perm)
    ...
    context.compute((0, conv2d_mm_1.createConv2DMatMulProgramInfoLoader)(convInputs, adjustedAttributes, outputShape, dimAOuter, dimBOuter, dimInner, hasBias, sequentialAccessByThreads), { inputs: convInputs });
};
const conv1d = (context, attributes) => {
    ...
    context.compute((0, conv_grouped_1.createGroupedConvProgramInfoLoader)(inputs, adjustedAttributes, outputShape => isChannelLast ? [outputShape[0], outputShape[2], outputShape[3]] : []));
};
const conv = (context, attributes) => {
